...
Component({
  ...
  methods: {
     ...
     */
    updated(sliderValue) {
      const offsets = sliderValue.map(value =>
        this.calcOffset(this.checkValue(value))
      );
    ...
    onTouchMove(e) {
      ...
      this.getRect(".ci-slider__rail").then(rect => {
        if (!rect || !this.isMoved) return;

        const diffX =
          ((this.moveX - this.startX) / rect.width) *
        ...
        if (sliderValue[index] !== currentValue) {
          ...
          this.triggerEvent("onChange", { offsets: nextOffsets, value });
        }
      });
    },
    /**
    ...
    onTouchEnd(e) {
      ...
      this.triggerEvent("onAfterChange", { offsets, value });
    },
    /**
     * 获取界面上的节点信息
     */
